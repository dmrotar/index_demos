def generate_data(size)
  (1..size).map do |i|
    ...
    [i, Forgery(:name).last_name, Forgery(:name).first_name, Forgery('address').phone, rand(18..80), Forgery('basic').color, "{#{countries.join(",")}}"]
  end
end
...
def run
  ...
  by_fav_color = data.sort_by{|r|r[5]}.map{|r| [r[5],r[0]]}

  by_last_name_and_first_name = data.sort_by{|r|[r[1],r[2]]}.map{|r| [r[1],r[2],r[0]]}
  by_age_and_fav_color = data.sort_by{|r|[r[4], r[5]]}.map{|r| [r[4], r[5],r[0]]}
  by_and_fav_color_and_age = data.sort_by{|r|[r[5], r[4]]}.map{|r| [r[5], r[4],r[0]]}
  ...
  save_csv("by_and_fav_color_and_age", by_and_fav_color_and_age)
end
